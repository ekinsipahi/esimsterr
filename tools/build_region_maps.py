...
def build(polygons: list[list[tuple[float, float]]], *, whole_world: bool = False,
          tolerance: float = SIMPLIFY, min_area: float = MIN_AREA_SHARE) -> str:
    ...
    offset_x = PADDING + (CANVAS - 2 * PADDING - width * scale) / 2
    offset_y = PADDING + (CANVAS - 2 * PADDING - height * scale) / 2
...
def main():
    ...
    for slug, members in REGIONS.items():
        ...
        (svg_dir / f"{slug}.min.svg").write_text(SVG.format(label=label, path=compact),
                                                 encoding="utf-8")
        written = f"{svg_dir.name}/{slug}.svg (+{len(compact)} compact)"
        if vector_dir:
            name = f"region_{slug.replace('-', '_')}.xml"
            (vector_dir / name).write_text(VECTOR.format(path=path), encoding="utf-8")
            written += f" + {name}"
