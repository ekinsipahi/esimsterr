...
# --- Region maps -------------------------------------------------------------
# Each region is drawn as the place it actually is, from real coastlines; see
# tools/build_region_maps.py. Inlined rather than served as an <img> so the
# outline inherits currentColor and follows the theme like every other icon.
REGION_MAP_DIR = os.path.join("img", "regions")
_region_cache: dict[str, str] = {}


def _region_path(slug: str, compact: bool = False) -> str:
    """The path data for a region, or "" if we have not drawn that one yet."""
    slug = (slug or "").strip().lower()
    if not slug or not slug.replace("-", "").isalnum():
        return ""
    key = f"{slug}.min" if compact else slug
    if key in _region_cache:
        return _region_cache[key]
    source = os.path.join(settings.BASE_DIR, "static", REGION_MAP_DIR, f"{key}.svg")
    data = ""
    if os.path.exists(source):
        with open(source, encoding="utf-8") as handle:
            match = re.search(r'<path d="([^"]+)"', handle.read())
...
@register.simple_tag
def region_flag(slug, size=34, cls=""):
    """A region's flag: the same object a country gets, in the same list.

    A country row carries a filled rectangle you recognise before reading the
    name. A region carrying a thin outline in a big square tile does not, and at
    the 30px these are drawn at, nine different coastlines all read as the same
    blue smudge -- which is what one globe on every card looked like, only after
    more work. So the region gets a flag: the same 4:3 rectangle, the same
    corner radius, the silhouette filled, the ground coloured so the tiles are
    told apart before the shape is even parsed.

    Inlined rather than an <img> so it needs no second request in a list of nine
    and inherits the page's own styling.
    """
    slug = (slug or "").strip().lower()
    if not slug or not slug.replace("-", "").isalnum():
        return icon("globe", size=size, cls=cls)
    source = os.path.join(settings.BASE_DIR, "static", REGION_MAP_DIR, f"{slug}.flag.svg")
    key = f"{slug}!flag"
    body = _region_cache.get(key)
    if body is None:
        body = ""
        if os.path.exists(source):
            with open(source, encoding="utf-8") as handle:
                raw = handle.read()
            colour = re.search(r'<rect [^>]*fill="([^"]+)"', raw)
            path = re.search(r'<path d="([^"]+)"', raw)
            if colour and path:
                body = (f'<rect width="40" height="30" fill="{colour.group(1)}"/>'
                        f'<path d="{path.group(1)}" fill="#fff" fill-opacity=".92"/>')
        _region_cache[key] = body
    if not body:
        return icon("globe", size=size, cls=cls)
    return mark_safe(  # noqa: S308 - generated by us from public-domain geometry
        f'<svg class="flag {cls}" width="{size}" height="{round(int(size) * 0.75)}" '
        f'viewBox="0 0 40 30" aria-hidden="true" focusable="false">{body}</svg>'
    )


@register.simple_tag
def region_map(slug, size=26, cls=""):
    """The region's outline. Falls back to the globe for one we have not drawn."""
    # A coastline drawn for a 30px tile is a few kilobytes of detail nobody can
    # see at 16px, and a listing page carries nine of them.
    path = _region_path(slug, compact=int(size) <= 20)
    if not path:
        return icon("globe", size=size, cls=cls)
    return mark_safe(  # noqa: S308 - generated by us from public-domain geometry
        f'<svg class="ico {cls}" width="{size}" height="{size}" viewBox="0 0 100 100" '
